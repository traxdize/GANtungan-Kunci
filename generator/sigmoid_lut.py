...
# Configuration
LUT_FILE_PATH = "output/sigmoid_lut_q1_7_8.csv"
HEX_FILE_PATH = "output/sigmoid_lut_mem.hex"
FRACTIONAL_BITS = 8
...
# Generate LUT
print(f"Generating Sigmoid LUT with {NUM_ENTRIES} entries...")
Z_float = np.linspace(INPUT_RANGE_MIN, INPUT_RANGE_MAX, NUM_ENTRIES)
sigmoid_float = sigmoid(Z_float)
sigmoid_fixed = float_to_fixed(sigmoid_float)

# Save to CSV
data = {
    'Address_Index': np.arange(NUM_ENTRIES),
    'Input_Z_Sample': Z_float,
    'Output_Sigmoid': sigmoid_float,
    'LUT_Value_Q1_7_8': sigmoid_fixed
}

df_lut = pd.DataFrame(data)
df_lut.to_csv(LUT_FILE_PATH, index=False, float_format='%.8f')
print(f"Saved Sigmoid LUT to: {LUT_FILE_PATH}")

# Convert to HEX file
with open(HEX_FILE_PATH, 'w') as f:
    for value in sigmoid_fixed:
