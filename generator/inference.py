...
# Default Fixed Point (can be overridden by args)
FRACTIONAL_BITS = 24
FRACTIONAL_SCALE = 1 << FRACTIONAL_BITS
...
def float_to_fixed(val):
    return int(val * FRACTIONAL_SCALE)
...
def q_tanh(x):
    ...
    if res >= 1.0: return FRACTIONAL_SCALE
    if res <= -1.0: return -FRACTIONAL_SCALE
...
if __name__ == "__main__":
    ...
    parser.add_argument("--frac_bits", type=int, default=24)
    args = parser.parse_args()

    FRACTIONAL_BITS = args.frac_bits
    FRACTIONAL_SCALE = 1 << FRACTIONAL_BITS

    print_separator(f"FIXED POINT GAN VERIFICATION (Q{32-FRACTIONAL_BITS}.{FRACTIONAL_BITS})")

    g2_layer, g3_layer, d2_layer, d3_layer = load_all_weights()

    # Inputs (Test Case: 0.5, -0.2)
    noise_1 = float_to_fixed(0.5)
    noise_2 = float_to_fixed(-0.2)
